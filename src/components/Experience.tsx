...
const Experience = () => {
  ...
  return <section className="py-20 px-4 bg-muted animate-fade-in" id="experience">
      <div className="max-w-4xl mx-auto relative">
        ...
        <div className="space-y-8">
          {experiences.map((exp, index) => <div key={index} className="relative">
              ...
              <Collapsible open={openIndex === index} onOpenChange={() => setOpenIndex(openIndex === index ? null : index)} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 relative z-10">
                <CollapsibleTrigger className="w-full p-6">
                  <div className="flex items-start gap-4">
                    ...
                    {exp.achievements.length > 0 && <ChevronDown className={`w-5 h-5 text-muted-foreground transition-transform ${openIndex === index ? "rotate-180" : ""}`} />}
                  </div>
                </CollapsibleTrigger>
                {exp.achievements.length > 0 && <CollapsibleContent>
